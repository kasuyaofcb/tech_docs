...
| ⑥ | Portal でサブスクリプションが見えるか確認 | 検索バーから「サブスクリプション」を開き、自分のサブスクリプション（例: `Azure subscription 1`）が一覧表示されればOK |

> 🚨 **複数 Microsoft アカウントセッションの干渉**: ⑥でサブスクリプションが「ありません」表示になる / アクセス拒否のエラーが出る場合、ブラウザに別の Microsoft アカウントのセッションが残っていて、別組織のテナントを参照している可能性がある。**新しいシークレットウィンドウで②からやり直す**のが確実。詳しくは [azure.md §3](azure.md) の「複数アカウントセッションの干渉」参照。
